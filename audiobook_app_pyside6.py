...
from PySide6.QtWidgets import (QSlider, QWidgetAction, QComboBox, QApplication, QMainWindow, QPushButton, 
                               QVBoxLayout, QFileDialog, QLineEdit, QLabel, QWidget, QMessageBox, 
                               QHeaderView, QProgressBar, QHBoxLayout, QTableWidget, QTableWidgetItem)
...
class AudiobookMaker(QMainWindow):
    ...
    def __init__(self):
        ...
        self.media_player_connected = False  # Add this flag
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    def play_selected_audio(self):
        ...
        self.media_player = QMediaPlayer()  # Reset media player
        self.audio_output = QAudioOutput()
        self.media_player.setAudioOutput(self.audio_output)
        ...
        self.media_player.sourceChanged.connect(self.on_audio_finished)
        self.media_player_connected = True  # Update the flag
    ...
    def play_all_from_selected(self):
        ...
        # Ensure the media player is initialized/reset
        try:
            if self.media_player_connected:
                self.media_player.mediaStatusChanged.disconnect(self.on_audio_finished)
                self.media_player_connected = False
        except TypeError:
            pass  # Handle if it wasn't connected

        self.pause_button.setStyleSheet("")
        self.media_player = QMediaPlayer()  # Re-initialize media player
        self.audio_output = QAudioOutput()
        self.media_player.setAudioOutput(self.audio_output)
        ...
        if idx < self.tableWidget.rowCount():
            ...
            if item:
                ...
                if map_key in self.text_audio_map:
                    ...
                    # Reset and set up the media player
                    self.media_player = QMediaPlayer()  # Reset media player
                    self.audio_output = QAudioOutput()
                    self.media_player.setAudioOutput(self.audio_output)
                    ...
                    try:
                        self.media_player.mediaStatusChanged.disconnect(self.on_audio_finished)
                        self.media_player.sourceChanged.disconnect(self.on_audio_finished)
                    except TypeError:
    ...
    def on_audio_finished(self, state):
        print(f"Audio finished with state: {state}")
        if state == QMediaPlayer.EndOfMedia:
            print(f"Playing next audio, current index: {self.current_sentence_idx}")
            self.current_sentence_idx += 1
            if self.current_sentence_idx < self.tableWidget.rowCount():
                self.play_audio_by_index(self.current_sentence_idx)
            else:
                print("Finished playing all selected audios")
                try:
                    self.media_player.mediaStatusChanged.disconnect(self.on_audio_finished)
                except TypeError:
                    pass
    ...
    def regenerate_audio_for_sentence(self):
        ...
        # Stop the media player and clear the media source if it's using the current file
        if self.media_player.playbackState() == QMediaPlayer.PlayingState:
